...
CBitmapEx::CBitmapEx(const CBitmapEx& bitmapEx){
	// Init members
	m_lpData = NULL;
	m_dwSize = 0;
	m_iPaletteEntries = 0;
...
CBitmapEx& CBitmapEx::operator =(const CBitmapEx& bitmapEx){
	this->~CBitmapEx();
	// Init members
	m_lpData = NULL;
	m_dwSize = 0;
...
void CBitmapEx::Create(const CBitmapEx& bitmapEx)
{
	//std::lock_guard<std::mutex> locker(mtx);
	...
	if (bitmapEx.IsValid())
	{
		...
		memcpy(&m_bfh, &bitmapEx.m_bfh, sizeof(BITMAPFILEHEADER));
		memcpy(&m_bih, &bitmapEx.m_bih, sizeof(BITMAPINFOHEADER));
		...
		memcpy(m_lpPalette, &bitmapEx.m_lpPalette, m_iPaletteEntries*sizeof(RGBQUAD));
...
void CBitmapEx::BilateralBlur(int num)	//双边滤波，计算时没有算中间那个点
{
	// Check for valid bitmap
	if (IsValid())
	{
		...
		//距离权重数组
		double* disWeight = (double*)malloc((2 * num + 1)*(2 * num + 1)*sizeof(double));
		//颜色权重数组
		double* r_Weight = (double*)malloc((2 * num + 1)*(2 * num + 1)*sizeof(double));
		double* g_Weight = (double*)malloc((2 * num + 1)*(2 * num + 1)*sizeof(double));
		double* b_Weight = (double*)malloc((2 * num + 1)*(2 * num + 1)*sizeof(double));

		//	int N = (2 * num + 1)*(2 * num + 1) - 1;	//方格内除中心点总点数
		int N = (2 * num + 1)*(2 * num + 1);	//方格内总点数
		...
		for (long i = 0; i < m_bih.biHeight; i++)
		{
			...
			for (long j = 0; j < m_bih.biWidth; j++)
			{
				...
				// Update bitmap
				DWORD dwSrcOffset = dwTotalOffset;
				DWORD cenSrcOffset = i*m_iPitch + j*m_iBpp;	//中心点块位置
				//		fixed f_red = 0, f_green = 0, f_blue = 0;

				double eucDis_R = 0;	//(red目标-red平均值)^2求和
				double eucDis_G = 0;
				double eucDis_B = 0;
				double ave_R = 0;
				double ave_G = 0;
				double ave_B = 0;
				double sumWeight_R = 0;	//R分量总权重
				double sumWeight_G = 0;
				double sumWeight_B = 0;

				double sum_R = 0;	//R分量值总和
				double sum_G = 0;	//R分量值总和
				double sum_B = 0;	//R分量值总和

				//计算周围像素RGB平均值
				for (long k = -num; k <= num; k++)
				{
					int count = 0;
					...
					for (long l = -num; l <= num; l++)
					{
						...
						if (n >= m_bih.biWidth - 1)
							n = m_bih.biWidth - 1;
						dwSrcOffset = m*m_iPitch + n*m_iBpp;	//目标点在图像中的块位置
						ave_R += _GetRValue(lpSrcData[dwSrcOffset >> 2]);
						ave_G += _GetGValue(lpSrcData[dwSrcOffset >> 2]);
						ave_B += _GetBValue(lpSrcData[dwSrcOffset >> 2]);
						count++;
					}
				}
				ave_R /= N;
				...
				for (long k = -num; k <= num; k++)
				{
					...
					for (long l = -num; l <= num; l++)
					{
						...
						if (n >= m_bih.biWidth - 1)
							n = m_bih.biWidth - 1;
						dwSrcOffset = m*m_iPitch + n*m_iBpp;	//目标点在图像中的块位置
						r_Weight[count] = (_GetRValue(lpSrcData[dwSrcOffset >> 2]) - _GetRValue(lpSrcData[cenSrcOffset >> 2]))*((_GetRValue(lpSrcData[dwSrcOffset >> 2]) - _GetRValue(lpSrcData[cenSrcOffset >> 2])));
						g_Weight[count] = (_GetGValue(lpSrcData[dwSrcOffset >> 2]) - _GetGValue(lpSrcData[cenSrcOffset >> 2]))*((_GetGValue(lpSrcData[dwSrcOffset >> 2]) - _GetGValue(lpSrcData[cenSrcOffset >> 2])));
						b_Weight[count] = (_GetBValue(lpSrcData[dwSrcOffset >> 2]) - _GetBValue(lpSrcData[cenSrcOffset >> 2]))*((_GetBValue(lpSrcData[dwSrcOffset >> 2]) - _GetBValue(lpSrcData[cenSrcOffset >> 2])));

						//		eucDis_R += r_Weight[count];
						//		eucDis_G += g_Weight[count];
						//		eucDis_B += b_Weight[count];


						eucDis_R += (_GetRValue(lpSrcData[dwSrcOffset >> 2]) - ave_R)*((_GetRValue(lpSrcData[dwSrcOffset >> 2]) - ave_R));
						eucDis_G += (_GetGValue(lpSrcData[dwSrcOffset >> 2]) - ave_G)*((_GetGValue(lpSrcData[dwSrcOffset >> 2]) - ave_G));
						eucDis_B += (_GetBValue(lpSrcData[dwSrcOffset >> 2]) - ave_B)*((_GetBValue(lpSrcData[dwSrcOffset >> 2]) - ave_B));
						count++;
					}
				}
				double t_R = eucDis_R / N; //标准差的平方
				double t_G = eucDis_G / N;
				double t_B = eucDis_B / N;
				...
				for (long k = -num; k <= num; k++)
				{
					...
					for (long l = -num; l <= num; l++)
					{
						...
						{
							disWeight[count] = 2;
							r_Weight[count] = t_R == 0 ? 1 : exp(-0.5*r_Weight[count] / t_R);
							g_Weight[count] = t_R == 0 ? 1 : exp(-0.5*g_Weight[count] / t_G);
							b_Weight[count] = t_R == 0 ? 1 : exp(-0.5*b_Weight[count] / t_B);

							sum_R += disWeight[count] * r_Weight[count] * _GetRValue(lpSrcData[dwSrcOffset >> 2]);
							sum_G += disWeight[count] * g_Weight[count] * _GetGValue(lpSrcData[dwSrcOffset >> 2]);
							sum_B += disWeight[count] * b_Weight[count] * _GetBValue(lpSrcData[dwSrcOffset >> 2]);
							sumWeight_R += disWeight[count] * r_Weight[count];
							sumWeight_G += disWeight[count] * g_Weight[count];
							sumWeight_B += disWeight[count] * b_Weight[count];
							count++;
							continue;
						}
				...
				BYTE red = (BYTE)max(0, min((int)(sum_R / sumWeight_R), 255));
				BYTE green = (BYTE)max(0, min((int)(sum_G / sumWeight_G), 255));
				BYTE blue = (BYTE)max(0, min((int)(sum_B / sumWeight_B), 255));
				lpDstData[dwTotalOffset >> 2] = _RGB(red, green, blue);
...
_PIXEL CBitmapEx::GetPixel(long x, long y) const
{
...
void CBitmapEx::RotateCircle(long degress)
{
	int widthbef, widthaft, dif;
	CBitmapEx other;

	widthbef = m_bih.biWidth;
	other.Create(widthbef, widthbef);
	Rotate(degress);
	widthaft = m_bih.biWidth;
	dif = (widthaft - widthbef) / 2;

	int i, j;
	for (i = 0; i < widthbef; i++)
	{
		for (j = 0; j < widthbef; j++)
		{
			_PIXEL pix = GetPixel(i + dif, j + dif);
			other.SetPixel(i, j, pix);
		}
	}
	Create(other);
	other.~CBitmapEx();
...
void CBitmapEx::FillBlackWhite(int level)//黑白bmp边缘填充
{
	_PIXEL black = _RGB(0, 0, 0);
	_PIXEL white = _RGB(255, 255, 255);
	int i, j, m;
	for (m = 0; m < level; m++)
	{
		for (i = 1; i < m_bih.biHeight; i++)
		{
			for (j = 1; j < m_bih.biWidth; j++)
			{
				_PIXEL pix[9];
				pix[0] = GetPixel(i, j);
				pix[1] = GetPixel(i - 1, j - 1);
				pix[2] = GetPixel(i - 1, j);
				pix[3] = GetPixel(i - 1, j + 1);
				pix[4] = GetPixel(i, j - 1);
				pix[5] = GetPixel(i, j + 1);
				pix[6] = GetPixel(i + 1, j - 1);
				pix[7] = GetPixel(i + 1, j);
				pix[8] = GetPixel(i + 1, j + 1);
				if (pix[0] == white&&pix[1] == black&&pix[2] == black&&pix[3] == black&&pix[4] == black&&pix[5] == black
					&&pix[6] == black&&pix[7] == black&&pix[8] == black)
				{
					SetPixel(i, j, black);
				}
				if (pix[0] == black)
...
void CBitmapEx::LineTo(int x1, int y1, int x2, int y2, DWORD color)

{
	...
	p2.X = x2;
	p2.Y = y2;
	if (p1.X < 0 || p1.X >= m_bih.biWidth || p1.Y < 0 || p1.Y >= m_bih.biHeight || p2.X < 0 || p2.X >= m_bih.biWidth || p2.Y < 0 || p2.Y >= m_bih.biHeight)
		return;
	int w = p1.X > p2.X ? (p1.X - p2.X) : (p2.X - p1.X);
	int h = p1.Y > p2.Y ? (p1.Y - p2.Y) : (p2.Y - p1.Y);
	if (w > h)
	{
		double step = (double)h / w;
		for (int i = 0; i <= w; i++)
		{
			int width = p2.X > p1.X ? i : -i;
			int j = (int)(i*step + 0.5);
			int height = p2.Y > p1.Y ? j : -j;
			SetPixel(p1.X + width, p1.Y + height, color);
		}
	}
	else
	{
		double step = (double)w / h;
		for (int i = 0; i <= h; i++)
		{
			int height = p2.Y > p1.Y ? i : -i;
			int j = (int)(i*step + 0.5);
			int width = p2.X > p1.X ? j : -j;
			SetPixel(p1.X + width, p1.Y + height, color);
		}
	}*/
}

void CBitmapEx::FillWhiteKNN()
{
	CBitmapEx bitmapOther;
	bitmapOther.Create(900, 900);
	bitmapOther.Clear();
	bitmapOther.SetResampleMode(RM_BICUBIC);
	double sumB = 0;
	double sumW = 0;
	double step = 0.5;
	for (int j = 1; j < m_bih.biHeight - 1; j++)
	{
		for (int i = 1; i < m_bih.biWidth - 1; i++)
		{
			if (GetPixel(i, j) == 0)
			{
				...
				if (sumW >= sumB)
					bitmapOther.SetPixel(i, j, _RGB(Mid_R_FromAround(i, j), Mid_R_FromAround(i, j), Mid_R_FromAround(i, j)));

			}
			else
			{
				bitmapOther.SetPixel(i, j, GetPixel(i, j));
			}
			sumB = 0;
			sumW = 0;
		}
	...
	Create(&bitmapOther);
	bitmapOther.~CBitmapEx();
	//	bitmapOther.Save(_T("C:\\Users\\zjj\\Desktop\\fillMid.bmp"));
}

//耗时操作
void CBitmapEx::FillWithKNN(DWORD color)
{
//	CBitmapEx bitmapOther;
//	bitmapOther.Create(m_bih.biWidth, m_bih.biHeight);
//	bitmapOther.Clear();
//	bitmapOther.SetResampleMode(RM_BICUBIC);
	double sumB = 0;
	double sumW = 0;
...
int CBitmapEx::Ave_R_FromAround(int x, int y)
{
	if (x >= 0 && x < m_bih.biWidth&&y >= 0 && y < m_bih.biHeight)
	{
		int i = 0;
		int sum = 0;
		if (x - 1 >= 0)
		{
			if (y - 1 >= 0)
			{
				sum += GetPixel(x - 1, y - 1) % 256;
				i++;
				sum += GetPixel(x, y - 1) % 256;
				i++;
			}
			sum += GetPixel(x - 1, y) % 256;
			i++;
			sum += GetPixel(x, y) % 256;
			i++;
			if (y + 1 < m_bih.biHeight)
			{
				sum += GetPixel(x - 1, y + 1) % 256;
				i++;
				sum += GetPixel(x, y + 1) % 256;
...
DWORD CBitmapEx::Ave_RGB_FromAround(int x, int y)
{
	if (x >= 0 && x < m_bih.biWidth&&y >= 0 && y < m_bih.biHeight)
	{
		int i = 0;
...
int CBitmapEx::Mid_R_FromAround(int x, int y)
{
#define maxNum 9
	int R[maxNum] = { 0 };
	int i = 0;
	if (x >= 0 && x < m_bih.biWidth&&y >= 0 && y < m_bih.biHeight)
	{
		if (x - 1 >= 0)
		{
			if (y - 1 >= 0)
			{
				R[i++] = GetPixel(x - 1, y - 1) % 256;
				R[i++] = GetPixel(x, y - 1) % 256;
			}
			R[i++] = GetPixel(x - 1, y) % 256;
			R[i++] = GetPixel(x, y) % 256;
			if (y + 1 < m_bih.biHeight)
			{
				R[i++] = GetPixel(x - 1, y + 1) % 256;
				R[i++] = GetPixel(x, y + 1) % 256;
...
void CBitmapEx::smooth()
{
	CBitmapEx other;
	other.Create(m_bih.biWidth, m_bih.biHeight);
	for (int i = 0; i < m_bih.biWidth; i++)
	{
		for (int j = 0; j < m_bih.biHeight; j++)
		{
			other.SetPixel(i, j, _RGB(Ave_R_FromAround(i, j), Ave_R_FromAround(i, j), Ave_R_FromAround(i, j)));
			//	other.SetPixel(i, j, Ave_RGB_FromAround(i,j));
		}
	}
	other.Save(_T("C:\\Users\\zjj\\Desktop\\smoothAve_RGB.bmp"));
}

double e_4x2(double x)
{
	return 0.1*exp(-4 * x*x);
}

void CBitmapEx::rainClutter(int x, int y, int Rmax)
{
	unsigned int weith = Rmax * 2 + 1;
	for (int i = x - Rmax; i <= x + Rmax;)
	{
		for (int j = y - Rmax; j <= y + Rmax; j++)
		{
			double R = sqrt((double)((i - x)*(i - x) + (j - y)*(j - y)));
			if (R > Rmax) continue;
			double m = R / (double)Rmax;
			double pro = e_4x2(m);	//概率
			unsigned int tickcount = GetTickCount();
			srand(tickcount + i*weith + j*j);
			int ran = rand() % 1000;
			if (ran <= pro * 1000)
			{
				int color = (int)(5100.0*pro + 0.5);
				if (color > 255) color = 255;
				SetPixel(i, j, _RGB(color, color, color));
				if (rand() % 1000 < 500)
					SetPixel(i - 1, j - 1, _RGB(color, color, color));
				if (rand() % 1000 < 500)
					SetPixel(i, j - 1, _RGB(color, color, color));
				if (rand() % 1000 < 500)
					SetPixel(i + 1, j - 1, _RGB(color, color, color));
				if (rand() % 1000 < 500)
					SetPixel(i, j - 1, _RGB(color, color, color));
				...
				if (rand() % 1000 < 500)
					SetPixel(i + 1, j - 1, _RGB(color, color, color));
				if (rand() % 1000 < 500)
					SetPixel(i + 1, j, _RGB(color, color, color));
				if (rand() % 1000 < 500)
					SetPixel(i + 1, j + 1, _RGB(color, color, color));
			}
			j += rand() % 12;
...
//void CBitmapEx::UnPackPSD(FILE *fp,	// fp为PSD文件指针，
//	DWORD dwWidth,
//	DWORD dwHeight,
...
//HRESULT CBitmapEx::LoadPSD(LPSTR strFilename, CRect m_Rect) // 读取PSD文件
//{
