...
public:
	// Public methods
	CBitmapEx();
	CBitmapEx(const CBitmapEx& bitmapEx);
	CBitmapEx& operator =(const CBitmapEx& bitmapEx);
	virtual ~CBitmapEx();
	void Create(long width, long height);
	void Create(const CBitmapEx& bitmapEx);
	...
	void RotateCircle(long degress = 0);
	...
	long GetWidth()const { return m_bih.biWidth; }
	long GetHeight()const { return m_bih.biHeight; }
	long GetPitch() const{ return m_iPitch; }
	long GetBpp() const{ return (m_iBpp << 3); }
	long GetPaletteEntries() const{ return m_iPaletteEntries; }
	LPRGBQUAD GetPalette() { return m_lpPalette; }
	DWORD GetSize() const{ return m_dwSize; }
	LPBYTE GetData() const{ return m_lpData; }
	void SetResampleMode(_RESAMPLE_MODE mode = RM_NEARESTNEIGHBOUR) { m_ResampleMode = mode; }
	_RESAMPLE_MODE GetResampleMode() const{ return m_ResampleMode; }
	BOOL IsValid()const { return (m_lpData != NULL); }
	void SetPixel(long x, long y, _PIXEL pixel);
	_PIXEL GetPixel(long x, long y) const;
	...
	void FillBlackWhite(int level = 2);//黑白bmp边缘填充
	void LineTo(int x1, int y1, int x2, int y2, DWORD color);
	void FillWhiteKNN();	//KNN分类
	void FillWithKNN(DWORD color);	//用color来填补空隙，默认背景为黑色
	int Ave_R_FromAround(int x, int y);	//从9*9获得中心点均值Red（均值滤波）
	DWORD Ave_RGB_FromAround(int x, int y);	//从9*9获得中心点均值RGB
	int Mid_R_FromAround(int x, int y);	//从9*9获得中心点中值Red（中值滤波）
	void smooth();
	void BilateralBlur(int num = 1/*范围*/);	//双边滤波

	void rainClutter(int x, int y, int Rmax);

	void UnPackPSD(FILE *fp,	// fp为PSD文件指针，
		DWORD dwWidth,
		DWORD dwHeight,
	...
	HRESULT LoadPSD(LPSTR strFilename, CRect m_Rect); // 读取PSD文件
