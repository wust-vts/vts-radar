...
struct aivdm_context_t *ais_context, struct ais_t *ais)
	//该函数既是AIVDM的比较也是AIVDO的比较，因为两者完全一样，除了最开始的"!AIVDM"和"!AIVDO"之外
	//而该函数的解析撇除了这两者的差异，即不区分该信息是目标船还是本船的,唯一区别的地方就是本船时
	//人为设定其MMSI为1。
{
	char *sixbits[64] = {
	...
	//sprintf(PARSEDINFO,"共有%d份AIS信息,这是第%d份, 该AIS初始信息是：%s\n",ais_context->await, ais_context->part, data);
...
#define UCHARS(s, to)	from_sixbit((char *)ais_context->bits, s, sizeof(to), to)
		...
		//每条AIVDM信息都包含这三种信息：信息类型（26中某一类）、重复数、MMSI号
		ais->type = UBITS(0, 6);
		ais->repeat = UBITS(6, 2);
		ais->mmsi = UBITS(8, 30);
		if (buf[5] == 'O')//如果是!AIVDO
			ais->mmsi = 1;
		//sprintf(PARSEDINFO,"AIVDM 信息类型 %d, MMSI %09d:\n",ais->type, ais->mmsi);
		/*
		...
		case 3:
			/*if (ais_context->bitlen != 168) {
				warnStr.Format("warning:AIVDM 消息类型 %d 大小不是168位(%zd).",
				ais->type,
				ais_context->bitlen);
		...
		case 5: // Ship static and voyage related data 
			...
			/*case 6: //Addressed Binary Message
				...
				warnStr.Format("warning:AIVDM 消息类型 %d 大小越界 (%zd).",
				ais->type,
				ais_context->bitlen);
				AfxMessageBox(warnStr);
				...
				SENDMSG_AISINFO;
				break;*/
		case 18:	// Standard Class B CS Position Report 较1-3类型简单点，功能上少了status和ROT
			/*if (ais_context->bitlen != 168) {
		...
		case 24:	// Class B CS Static Data Report 
			switch (UBITS(38, 2)) {//part number,如果是0则以part A解析，否则按照part B解析
			case 0:
				if (ais_context->bitlen != 160) {
...
#undef UBITS
	...
	//如果当前给出的buf不是完整的一条AIS信息，则返回false表示失败，通过本次的part和await信息判断失败的原因
	//是该AIS信息被分割了几个语句后，保留本次的ais_context信息继续调用本函数进行解析即可
